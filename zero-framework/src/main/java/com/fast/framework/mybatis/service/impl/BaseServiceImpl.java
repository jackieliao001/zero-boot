package com.fast.framework.mybatis.service.impl;
...
import com.fast.framework.mybatis.interceptor.DataScope;
...
public class BaseServiceImpl<K extends BaseMapper<T>,T> extends ServiceImpl<K,T> implements BaseService<T> {
    ...
    protected DataScope getDataScope(String tableAlias, String orgIdAlias) {
        ...
        // 如果是超级管理员，则不进行数据过滤
        if (user.getSuperAdmin().equals(Constant.SUPER_ADMIN_ID)) {
            return null;
        }
